...
for label in elapsed:
  ...
  for thread_count in sort(elapsed[label].keys()):
    ...
    test_length = (test_end - test_start) / 1000.0
  ...
  ax1.set_yscale('log')
  bp = boxplot(plot_data, notch=0, sym='+', vert=1, whis=1.5, showmeans=1)
  ...
  ax2.set_ylabel('Throughput (Samples per second)')
  ax1.set_ylabel('Response Time (millisenconds)')
